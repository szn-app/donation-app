...
install@oathkeeper() {
    ...
    helm.install@oathkeeper() {
        ...
        l="$(mktemp).log" && helm upgrade --debug --install oathkeeper ory/oathkeeper -n auth --create-namespace -f ./helm-values.yaml -f $t \
                --set-file oathkeeper.accessRules=$j > $l 2>&1 && printf "Oathkeeper helm rendering logs: file://$l\n"
...
delete.helm#ory-stack#manual-uninstall@ory-oathkeeper() {
...
verify@oathkeeper() { 
    ...
    kubectl get configmaps -n auth | grep oathkeeper
